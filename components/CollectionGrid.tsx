...
const WatchCard: React.FC<{ watch: Watch; index: number }> = ({ watch, index }) => {
    return (
        <motion.div 
            ...
            className="group relative bg-luxury-card border border-white/5 hover:border-luxury-gold/50 transition-colors duration-500 overflow-hidden"
        >
            {/* Image Container */}
            <div className="aspect-[4/5] relative overflow-hidden bg-[#1a1a20]">
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                    {watch.status !== 'AVAILABLE' && (
                        <span className={`px-3 py-1 text-[9px] font-bold uppercase tracking-widest ${
                            watch.status === 'SOLD' ? 'bg-luxury-red text-white' : 'bg-orange-500 text-black'
                        }`}>
                            {watch.status}
                        </span>
                    )}
                ...
                {/* Like Button */}
                <button className="absolute top-4 left-4 text-white/50 hover:text-white transition-colors z-20">
            ...
            <div className="p-6 relative">
                ...
                <div className="flex justify-between items-end pt-4 border-t border-white/5">
                    ...
                    <button className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-all">
